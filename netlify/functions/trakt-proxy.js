...
exports.handler = async (event) => {
    ...
    if (httpMethod === 'GET' && path.endsWith('/download-spreadsheet')) {
        try {
            ...
            // **NOVO BLOCO DE ERRO: Isolando a chamada Axios para capturar erros silenciosos**
            let response;
            try {
                ...
                // Em caso de erro HTTP (4xx, 5xx), a propriedade `response` existe
                if (axiosError.response) {
                    console.error("Erro da API da Trakt:", axiosError.response.status, axiosError.response.data);
                }
                
                // Repassa o erro para o bloco catch principal
                throw axiosError;
            }
